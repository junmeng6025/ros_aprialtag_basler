        ...
        for label in values:
            ...
            if value_str.find('\n') > 0:
                ...
                value_str = [value_str[0]] + \
                    [indent_str+v for v in value_str[1:]]
                value_str = '\n'.join(value_str)
...
class Detector(object):
    ...
    def __init__(self, options=None, searchpath=[]):
        ...
        if options.families == 'all':
            ...
            families_list = [n for n in re.split(
                r'\W+', options.families) if n]
    ...
    def _declare_return_types(self):
        ...
        self.libc.apriltag_detector_create.restype = ctypes.POINTER(
            _ApriltagDetector)
        self.libc.apriltag_family_create.restype = ctypes.POINTER(
            _ApriltagFamily)
...
def _get_dll_path():
    ...
    return [
        os.path.join(os.path.dirname(__file__), '../apriltag_src/build/lib'),
        os.path.join(os.getcwd(), '../apriltag_src/build/lib')
    ]
...
def detect_tags(image,
    ...
    result = []
    poses = []
    numpy.set_printoptions(suppress=True, formatter={
    ...
    for i, detection in enumerate(detections):
        ...
        if vizualization == 1:
            ...
            _draw_pose_axes(overlay, camera_params,
                            tag_size, pose, detection.center)
